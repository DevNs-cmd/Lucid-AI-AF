...
exports.env = {
    ...
    STORY_AI_PROVIDER: (process.env.STORY_AI_PROVIDER || "anthropic"),
    IMAGE_AI_PROVIDER: (process.env.IMAGE_AI_PROVIDER || "openai"),
    VOICE_AI_PROVIDER: (process.env.VOICE_AI_PROVIDER || "elevenlabs"),
    EMBEDDING_AI_PROVIDER: (process.env.EMBEDDING_AI_PROVIDER || "openai"),
    // Payments
...
function requireKey(value, name) {
    if (!value) {
        throw new Error(`[LUCID AI] Missing "${name}". Add it to your .env once the key is issued.`);
    }
    return value;
}
