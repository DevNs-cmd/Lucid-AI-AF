...
import { VoiceGenRequest, VoiceGenResult, TranscribeRequest, TranscribeResult } from "./types";

/**
 * Voice AI — NPC text-to-speech (ElevenLabs/Cartesia) and
 * speech-to-text for voice input (Whisper).
 */
export async function generateVoice(req: VoiceGenRequest): Promise<VoiceGenResult> {
...
async function callElevenLabs(req: VoiceGenRequest): Promise<string> {
  const apiKey = requireKey(env.ELEVENLABS_API_KEY, "ELEVENLABS_API_KEY");
...
async function callCartesia(req: VoiceGenRequest): Promise<string> {
  const apiKey = requireKey(env.CARTESIA_API_KEY, "CARTESIA_API_KEY");
